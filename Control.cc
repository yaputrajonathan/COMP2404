...
Control::Control()
{
  ...
  hotel = new Hotel("Jo's Hotel", resMgr);
  resMgr->setHotel(hotel);

  StayRecorder* stayRec = new StayRecorder("stay");
  GuestRecorder* guestRec = new GuestRecorder("guest");
  UpgradeRecorder* upgradeRec = new UpgradeRecorder("upgrade");

  resMgr->subscribe(stayRec);
  resMgr->subscribe(guestRec);
  resMgr->subscribe(upgradeRec);
...
void Control::launch()
{
  ...
  while(choice != 0){
    ...
    switch (choice) {
      case 1: resMgr -> print();
              break;
      case 2: hotel -> printRooms();
              break;
      case 3: hotel -> printGuests();
              break;
      case 4: resMgr -> printRecords();
              break;
      default:
              break;
    }
